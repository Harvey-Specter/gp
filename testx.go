...
func xz3t3l(db *sql.DB, rqParam string) {
	fmt.Println("date==xz3t3l=" + rqParam)
	voltimes := 0.8 //1.8
	dms := getDm(db, rqParam)
	for _, dm := range dms {
		sps := []float64{}

		dmSql := "SELECT id, date rq,code dm, close sp, high zg, low zd, m5, volume as cjl,pre_close qsp , open as kp FROM dayline where code = '" + dm["code"].(string) + "'  ORDER BY code,date DESC"
		...
		for rows.Next() {
			...
			if cnt == 0 {
				if m5 == 0 || sp <= qsp || sp > qsp*1.03 || sp > qsp*1.01 { //|| sp < zg*0.95 {
					ok = false
					break
				} else {
					sps = append(sps, sp)
			...
			if cnt == 1 {
				...
				if cnt == 2 {
					// 暂时不考虑昨天的收盘价是否下跌1==2
					if m5 == 0 || sp < qsp || sp > qsp*1.03 || sp > qsp*1.01 { //这块不能删 && sp < m5 { //    cjl0 <= float64(cjl)*1.2 &&
