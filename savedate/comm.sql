...
CREATE TABLE `industry` (
  ...
  `inc_revenue_year_on_year` double NOT NULL DEFAULT 0,
...
invest_income_associates	对联营企业和合营企业的投资收益(元)	持有的对联营企业及合营企业的投资，按照《企业会计准则第2号——长期股权投资》的规定，应当采用权益法核算，在按持股比例等计算确认应享有或应分担被投资单位的净损益时，应当考虑以下因素：投资企业与联营企业及合营企业之间发生的内部交易损益按照持股比例计算归属于投资企业的部分，应当予以抵销，在此基础上确认投资损益。
...
select count(1), substr(code,5,2) from  dayline group by substr(code,5,2);
...
CREATE TABLE `tp` (
  ...
  `p1` double NOT NULL DEFAULT 0.0,
  `p2` double NOT NULL DEFAULT 0.0,
  `p3` double NOT NULL DEFAULT 0.0,
  `p4` double NOT NULL DEFAULT 0.0,
  `p5` double NOT NULL DEFAULT 0.0,
  PRIMARY KEY (`id`),
  KEY `date` (`date`,`code`),
